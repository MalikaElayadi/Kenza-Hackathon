...
export default function App() {
  ...
  const [data, setData] = useState<{ kpis?: Record<string, string>; orders?: unknown[]; escalations?: unknown[] }>({});
  const [loading, setLoading] = useState(false);

  async function refreshDashboard() {
    const [kpis, orders, escalations] = await Promise.all([fetch(`${API}/api/kpis`).then((r) => r.json()), fetch(`${API}/api/orders`).then((r) => r.json()), fetch(`${API}/api/escalations`).then((r) => r.json())]);
    setData({ kpis, orders, escalations });
  }
