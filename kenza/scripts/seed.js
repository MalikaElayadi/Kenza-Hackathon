...
async function seedProducts() {
    ...
    const products = await loadCSV('catalogue.csv');
    if (products.length !== 80) {
        throw new Error(`Expected 80 products, got ${products.length}`);
    }
    for (const product of products) {
        await client.query(`INSERT INTO products (ref, modele, famille, genre, couleur, taille, matiere, saison, prix_mad, stock, delai_reassort_jours, code_barre, poids_g)
...
async function seedClients() {
    ...
    const clients = await loadCSV('clients.csv');
    if (clients.length !== 120) {
        throw new Error(`Expected 120 clients, got ${clients.length}`);
    }
    for (const c of clients) {
        await client.query(`INSERT INTO clients (client_id, nom, telephone, ville, langue_preferee, premier_achat, nb_commandes, segment)
...
async function seedShippingRates() {
    ...
    const rates = await loadCSV('livraison.csv');
    if (rates.length !== 12) {
        throw new Error(`Expected 12 shipping rates, got ${rates.length}`);
    }
    for (const rate of rates) {
        await client.query(`INSERT INTO shipping_rates (ville, frais_mad, delai_heures, paiement_a_la_livraison, retrait_boutique)
       VALUES ($1, $2, $3, $4, $5)
...
async function seedPromotions() {
    ...
    const promos = await loadCSV('promotions.csv');
    if (promos.length !== 12) {
        throw new Error(`Expected 12 promotions, got ${promos.length}`);
    }
    for (const promo of promos) {
        await client.query(`INSERT INTO promotions (ref, modele, prix_normal_mad, prix_promo_mad, debut, fin, condition)
...
async function seedOrders() {
    ...
    const orders = await loadCSV('commandes.csv');
    if (orders.length !== 320) {
        throw new Error(`Expected 320 orders, got ${orders.length}`);
    }
    for (const order of orders) {
        await client.query(`INSERT INTO orders (commande_id, client_id, date, canal, statut, total_articles_mad, frais_livraison_mad, total_mad, ville_livraison, paiement, created_by)
...
async function seedOrderItems() {
    ...
    const items = await loadCSV('commandes-lignes.csv');
    if (items.length !== 449) {
        throw new Error(`Expected 449 order items, got ${items.length}`);
    }
    for (const item of items) {
        await client.query(`INSERT INTO order_items (commande_id, ref, modele, taille, quantite, prix_unitaire_mad)
       VALUES ($1, $2, $3, $4, $5, $6)
       ON CONFLICT DO NOTHING`, [
            item.commande_id,
            item.ref,
